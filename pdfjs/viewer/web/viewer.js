...
function scrollToPage (page) {
  PDFViewerApplication.pdfLinkService.page = page;
}
...
var validateFileURL;
{
  var HOSTED_VIEWER_ORIGINS = ['null', 'http://mozilla.github.io', 'https://mozilla.github.io', 'http://localhost:8080', 'http://10.0.2.55'];
  ...
  validateFileURL = function validateFileURL(file) {
    ...
    console.log('---URL 参数里的 file 字段会在这里做同源检测')
    ...
    try {
      ...
      if (origin !== viewerOrigin && protocol !== 'blob:') {
        console.log(origin, viewerOrigin)
...
function () {
  ...
  _createClass(PDFFindBar, [{
    ...
    value: function updateResultsCount() {
      ...
      /* 返回查询结果 */
      window.parent.searchResult(total);
...
function () {
  ...
  _createClass(PDFLinkService, [{
    ...
    value: function setHash(hash) {
      ...
      console.info('setHash---此方法在初始化的时候就会执行, 会将URL 中的 hash 字段拿出来解析, 并对预先定义的字段进行操作, 比如 page,zoom 等');
      console.info('setHash---我会在这里增加 highlight 判断的')
      ...
      if (hash.includes('=')) {
        ...
        if ('search' in params) {
          searchText = params['search'].replace(/"/g, '')
